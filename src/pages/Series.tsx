...
import { Search, Filter, Play, Loader2, ArrowRight, Clapperboard } from 'lucide-react';
...
export default function Series() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setSeriesList(Array.isArray(allSeries) ? allSeries : []);
      } catch (error) {
  ...
  const filteredSeries = seriesList.filter((s) => {
    ...
    return s.category_id === activeCategoryId;
  });

  const displaySeries = activeCategoryId === 'recent' ? filteredSeries.slice(0, 50) : filteredSeries;
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-10 pt-6 space-y-8">
        ...
        {displaySeries.length === 0 && (
          <div className="flex flex-col items-center justify-center py-32 text-zinc-600">
            ...
            <p className="font-medium text-xl text-zinc-400">لا توجد مسلسلات مطابقة للبحث</p>
          </div>
        )}
      </div>
    </div>
